...
- StonkFun pays every coin from a small set of platform wallets, listed in `lib/distributors.ts`: `5KXDF6QnqhBj72hDtJNkkpFaQVUfbFXNybMsp3DiK6tD` until 2026-09-20, and `HuBMeYW3aDn8BH65fo8xxbP4oiexyup8udzKyccgi8Ga` for most batches since. The classifier, the batch parser and the scheduled pull read that list. When payouts turn `probable` on a statement, check for a new distributor first. Evidence in `docs/RESEARCH.md` ("Second distributor").
